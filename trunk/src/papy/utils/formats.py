...
#def export(self, file_name =None, format ="dot"):
    #""" Exports the graph in specified format.
    #"""
    #file_name = (file_name or repr(self) + "." + format)
    #print file_name
    #self.log.info('%s exporting as a %s-file: %s' % (repr(self), format, file_name))
    #if format == "dot":
        #utils.pipeline_to_dot(self, file_name)
    #else:
        #self.log.error('%s does not know format %s' % (repr(self), format))
        #raise DaggerError('%s does not know format %s' % (repr(self), format))
